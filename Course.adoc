...
This course (https://github.com/oliviercailloux/Introduction-to-measurement-theory[source repository]) is based on Fred Roberts’ book, https://doi.org/10.1017/CBO9780511759871[Measurement Theory]. 
Multiple exercices come from his book.
...
* Ordered pair, sets thereof, domain stem:[𝒟], image stem:[cc "I"], field stem:[ℱ = 𝒟 ∪ cc "I"]
...
* Relates stem:[a] to stem:[c] through stem:[b]

=== Reflexivity
stem:[a R a]

=== Start exercices
See xref:Ex1.adoc[Exercices S1].
...
https://en.wikipedia.org/wiki/Helmut_Hasse[Hasse] diagram: stem:[a R b, a ≠ b], corresponds to a line descending from stem:[a] to stem:[b].

=== More exercices
Complete xref:Ex1.adoc[Exercices S1] apart from symmetry related aspects.
...
For many authors (including Roberts but excluding Halmos), the set on which stem:[R] is defined is exogenous, thus a relation is a pair stem:[(A, R)] with stem:[R ⊆ A²] (hence stem:[ℱ(R) ⊆ A]). This allows for the possibility that stem:[ℱ(R) ≠ A]. Completeness is then defined as stem:[∀ a ≠ b ∈ A: a R b ν b R a]. Similarly, other definitions (such as reflexivity) then differ from those given here (one consequence is that reflexivity no more follows from transitivity and symmetry). In this document, we assume stem:[A] is chosen equal to stem:[ℱ(R)], in which case the definitions coincide.

=== Exercices
xref:Ex1.adoc[Exercices S1], xref:Exam1.adoc[Intermediate control 2025] (xref:Exam1.pdf[PDF]).

Solutions to xref:Sol1.adoc[some of the exercices]; to the xref:Corr1.adoc[control] (xref:Corr1.pdf[PDF]).

== Sorts of preorders
=== Equivalence
An equivalence relation is a transitive (, reflexive) and symmetric relation.

- Each element in an equivalence relation has an associated equivalence class.
- The equivalence class of an element stem:[a] is denoted by stem:[a^*** = {b ∈ ℱ(R) | a R b}].
- The set of equivalence classes, called the quotient by stem:[R], is denoted by stem:[ℱ(R) // R]. It partitions (disjointly classifies) stem:[ℱ(R)].

=== Reduction of a preorder
...
* That ordering is a relation on its quotient by stem:[sym(R)] and is called the reduction of stem:[R]
* Formally, the reduction of stem:[R] is defined as stem:[R^*** = {(a^***, b^***) ∈ (ℱ(R) // sym(R))² | ∃ a ∈ a^***, b ∈ b^*** | a R b}].

// xref:Ex2.adoc[Exercices S2] up to “Composition itérée”.
...
(More usually called weak completeness)
...
=== Complete order
Transitive, reflexive, complete, antisymmetric
...
=== Exercices
See xref:Ex2.adoc[Exercices S2].

== From relations to numbers
=== Side-uniqueness, ontoness
* Right-unique: stem:[a R b ∧ a R c ⇒ b = c]
* A right-unique relation is a function from stem:[𝒟(R)] to stem:[cc "I"(R)]; we can write stem:[R(a)] to denote the single stem:[b ∈ cc "I"(R)] such that stem:[a R b].
* Onto stem:[Y] (right-total over stem:[Y]): stem:[Y = cc "I"(R)]
...
* Relation stem:[R] on stem:[A] homomorphic to relation stem:[T] on stem:[ℝ] through function stem:[f] from stem:[A] to stem:[ℝ]: stem:[a R b ⇔ f(a) T f(b)].
* stem:[R] is homomorphic to stem:[T] iff it is homomorphic to stem:[T] through some function stem:[f].
* stem:[f] describes the homomorphism from stem:[R] to stem:[T].

=== Representation theorem
* A theorem of the form: under such conditions on stem:[R], the system is homomorphic to stem:[T].
...
Given stem:[R] with finite field stem:[ℱ(R)], stem:[R] is a complete preorder iff it is homomorphic to stem:[≥].

* Proof via: stem:[a R b ⇒ R(b) ⊆ R(a) ⇒ #R(a) ≥ #R(b)], and stem:[¬aRb ⇒ R(a) ≠ R(b)] (thus stem:[¬aRb ⇒ #R(a) < #R(b)])
* Can we relax finiteness?
* Scale type: ordinal

=== Exercices
See xref:Ex3.adoc[Exercices S3].

== Appendix
=== Homomorphisms with operations
...
* Representation theorems of the form: under such conditions on stem:[(R, {⊙_i})], the system is homomorphic to stem:[(T, {α_i})].
...
* It can be viewed as a function from stem:[A × A] to stem:[A]; we can write stem:[a α b] to denote the single stem:[c ∈ cc "I"(α)] such that stem:[(a, b) α c].
* Examples: stem:[+], stem:[×] on stem:[ℕ].

If stem:[T] is restricted to the image of stem:[f], it is determined uniquely by stem:[f] and stem:[R], in other words, stem:[R] never corresponds to two relations stem:[T1 ≠ T2] through a single function stem:[f] when stem:[ℱ(T1) = ℱ(T2) = cc "I"(f)]. Proof: if R corresponds to T1 and T2 through f with ℱ(T1) = ℱ(T2) = 𝓘(f), then x T1 y iff a R b, for any a ∈ f-1(x), b ∈ f-1(y), iff x * T2 y thus T1 = T2.

=== Arithmetic and infinite sets
* Successor of stem:[s]: stem:[s ⋃ {s}]
* Axiom of infinity: stem:[∃] successor set stem:[A]
* stem:[ℕ]: intersection of all successor sets in stem:[A]
* Permits induction, which we use to define addition of zero, then addition of one, …
* Finite set: bijection with an element of stem:[ℕ]
...
=== Completed semiorder
≽ is a completed semiorder: complete and reflexive and satisfies
...
== Wrap-up
When facing a falsifiable or normative claim, think about:

* modelisation
* extent of the proofs
